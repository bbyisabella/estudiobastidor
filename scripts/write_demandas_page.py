...
export default function Demandas() {
  ...
  async function salvarNova(e) {
    ...
    await supabase.from('demandas').insert([payload]);
    setSaving(false);
    setModalOpen(false);
    resetForm();
    await carregar();
      ...
      <main style={styles.main}>
        ...
        <motion.div style={styles.tabs} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          {[
            ...
            <button key={tab.key} style={{ ...styles.tab, ...(activeTab === tab.key ? styles.tabActive : {}) }} onClick={() => setActiveTab(tab.key)}>
              {tab.label}
            </button>
          ))}
        </motion.div>
        ...
        <motion.div style={styles.metricsRow} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
          {activeTab === 'design' ? (
            <>
              <div style={styles.metricCard}><span style={styles.metricValue}>{totais.designTotal}</span><span style={styles.metricLabel}>Total</span></div>
            ...
            <>
              <div style={styles.metricCard}><span style={styles.metricValue}>{totais.socialTotal}</span><span style={styles.metricLabel}>Total</span></div>
...
const styles = {
  ...
  main: { flex: 1, padding: '32px 36px 48px', position: 'relative', overflowX: 'hidden' },
  backgroundGlow: { position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden' },
  ...
  tabActive: { background: '#18181B', color: '#FFFFFF' },
  metricsRow: { display: 'grid', gridTemplateColumns: 'repeat(6, minmax(0, 1fr))', gap: 12, position: 'relative', zIndex: 1, marginBottom: 18 },
  metricCard: { background: '#111113', border: '1px solid #27272A', borderRadius: 16, padding: '14px 16px', boxShadow: '0 10px 24px rgba(0, 0, 0, 0.18)' },
  ...
  skeletonWrap: { display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 16, position: 'relative', zIndex: 1 },
  ...
  kanbanBoard: { display: 'grid', gridTemplateColumns: 'repeat(6, minmax(280px, 1fr))', gap: 14, overflowX: 'auto', position: 'relative', zIndex: 1, paddingBottom: 8 },
  column: { minWidth: 280, borderRadius: 18, border: '1px solid #27272A', padding: 12, minHeight: 420 },
  ...
  emptyColumn: { background: '#0F0F11', color: '#A1A1AA', borderRadius: 12, padding: '24px 12px', textAlign: 'center', fontSize: 13 },
  card: { background: '#18181B', borderRadius: 14, padding: 14, border: '1px solid #27272A', cursor: 'pointer', boxShadow: '0 10px 26px rgba(0, 0, 0, 0.18)' },
  ...
  overlay: { position: 'fixed', inset: 0, background: 'rgba(5, 5, 7, 0.78)', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', zIndex: 1200 },
  ...
  modalBackdrop: { position: 'fixed', inset: 0, background: 'rgba(5, 5, 7, 0.78)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, zIndex: 1300 },
...
Path('src/pages/Demandas.js').write_text(code, encoding='utf-8')
print('rewritten with UTF-8')
